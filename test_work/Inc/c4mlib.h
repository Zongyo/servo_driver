 ...
 * @param CYCLE 降頻週期
 * @param HWREGADD 中斷週期調整暫存器位址Hardware Register to be writen to
 * adjust period between HWInt  this Frequence Reduce executor used.
 * @param 硬體暫存器位元組數或是0 當它為0時則不做週期更新。Bytes of Hardware
 * Register to be writen. If it is 0, do not adjust period.
 * @param PERIODLISTADD 存放可調週期的陣列The address of the list of adjustable
 * periods.
 */
#    define FREQREDU_LAY(FRSTR, TASKNUM, CYCLE, HWREGADD, REGBYTE,             \
                         PERIODMATADD)                                         \
        FreqReduStr_t FRSTR = {0};                                             \
        {                                                                      \
...
typedef struct {
    ...
    volatile uint8_t ExCount; //Triggered Counter 
    uint8_t* ExCountOut_p; //Triggered Counter Pointer
    uint8_t NextTaskNum;
...
#define RT_REG_IO_LAY(RTRIOSTR,NEXTTASKNUM,HWREGADD,BYTES,DATA_P)             \
    RealTimeRegIOStr_t RTRIOSTR={                                           \
        ...
        .Data_p      = DATA_P,                                               \
        .ExCountOut_p= (uint8_t*)&(RTRIOSTR.ExCount)                                  \
    }
...
#define RT_FLAG_IO_LAY(RTFIOSTR,NEXTTASKNUM,HWREGADD,MASK,SHIFT,DATA_P)       \
    RealTimeFlagIOStr_t RTFIOSTR={  /*define structure */                   \
        ...
        .ExCountOut_p= (uint8_t*)&(RTFIOSTR.ExCount)                                  \
    } 
/*-- rtpio section end -------------------------------------------------------*/
